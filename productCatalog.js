...
document.getElementById("typeButton").addEventListener('click', function(){
    api.searchProductsByType(document.getElementById('type').value).then(function(products){
...
document.getElementById("priceButton").addEventListener('click', function(){
    api.searchProductsByPrice(document.getElementById('price').value, 50).then(function(products){
        updateTable('similarTable', products);
    }).catch(function(error){
        alert(error);
    });
